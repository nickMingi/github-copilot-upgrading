...
>
> 각 단계(해당되는 경우)에는 Copilot 제안이 올바른 명령인지 검증할 수 있는 `Cheatsheet`도 포함되어 있습니다.
>
> 💡 다양한 프롬프트로 실험해 보세요. 인라인 채팅을 사용할 때 추가 프롬프트로 응답을 세밀하게 조정할 수 있습니다.
...
> [!NOTE]
> 왜 안 될 수 있나요? 레거시 Python에서 더 이상 지원되지 않는 `distribute`를 사용하므로 의존성 설치가 실패할 수 있습니다. Copilot에게 다른 설치 방법을 제안받으세요.
...
> [!NOTE]
> 비즈니스 로직상 일부 구성요소가 LLM에 명확히 드러나지 않을 수 있으니 주의하세요.

### 4. 프로젝트 설치

가상환경을 만들고 프로젝트를 설치하세요. `python setup.py develop`을 실행해 오류를 파악하세요.

- 오류 메시지를 Copilot Chat에 붙여넣고 원인과 해결책을 질문하세요.
...
> [!NOTE]
> 오류가 바로 이해되지 않거나 Python 버전이 구식임을 암시할 수 있습니다. 추가 질문을 통해 오류의 원인을 파악하고 전략을 세우세요. 프로젝트가 오래된 Python 버전임을 Copilot에 힌트로 제공해도 좋습니다.

### 6. try/except 오류 수정

기능 테스트를 활용해 집중할 _작업 단위(slice)_ 를 정하세요. 이번에는 try/except 오류를 수정합니다.

- Copilot에게 구식 Python 버전을 언급하며 실제 문제 해결을 요청하세요.
...
- 예) _"이 프로젝트는 Python 2.5로 작성되었고, Python 3.9에서 다음과 같은 오류가 발생합니다:"_ 라고 질문하며 테스트 출력을 첨부하세요.
- 간단한 해결책이 있을 수 있습니다. 하나를 선택해 바로 적용하고 검증하세요.
- 테스트를 실행해 변경 사항이 올바른지 확인하세요.

[!NOTE]
> 모든 테스트가 통과해야 합니다. 통과하지 않으면 Copilot에 테스트 출력을 질문해 도움을 받고, 이전 단계를 반복하세요.
...
- 테스트 함수로 작성할 수 있다면 클래스 대신 함수로 작성하세요.
...
레거시 코드는 `distribute`를 사용했지만, 여전히 `setup.py`와 `setuptools`를 사용합니다. 이제는 `pyproject.toml`로 패키징하세요.
...
레거시 프로젝트에는 자동화가 없습니다. GitHub Actions로 테스트와 검증을 자동화하세요. 목표는 모든 PR에서 워크플로우가 실행되어 테스트와 설치를 시도하는 것입니다.
...
> [!NOTE]
> 자동화는 현대 소프트웨어 개발의 핵심이지만, 완벽하게 만들려면 시간이 걸릴 수 있습니다. 인내심을 갖고 Copilot의 도움을 받아 오류를 해결하세요.
